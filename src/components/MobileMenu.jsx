...
function MobileMenu({ isOpen, onMobileMenuBtnClick }) {
    ...
    return (
        <div className={`fixed top-0 w-full z-[100] h-screen bg-black lg:hidden transform ${menuTransform} transition-transform duration-500 ease-in-out`}>
            <div className=" w-screen px-5 border-b-[.8px] border-zinc-400 lg:px-12 py-6 flex justify-between bg-gray-300   duration-500 ease-[cubic-bezier(.3,.86,.36,.95)] bg-opacity-0 backdrop-filter backdrop-blur-md text-white" id='navbar' >
                ...
                <button onClick={onMobileMenuBtnClick} className='w-10 h-10  text-white flex justify-center items-center lg:hidden'>
                    <RxCross2 size={25} />
                </button>
            </div>
