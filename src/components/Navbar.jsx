...
const AnimatedLink = ({ Content, key }) => {
    return (
        <div className={`link-container overflow-hidden  h-[25px] group ${Content == "Contact Us" && "ml-32"} `}>
            <a className="link relative inline-block  text-[#212121]  " href="#">
                <div className='overflow-hidden '>
                    <h1 className="group-hover:translate-y-[-100%] transition-transform duration-300">
                        {Content}
                    </h1>
                    <h1 className="duplicate absolute group-hover:translate-y-[-100%] top-full left-0 duration-300">
                        {Content}
                    </h1>
                </div>
                <div className="underline absolute bottom-0 left-0 w-full h-[.1px] bg-[#212121] ease-linear origin-left transform scale-x-0 transition-transform duration-300 group-hover:scale-x-100 ">
                </div>
            </a>
        </div>
    );
...
function Navbar() {
    ...
    return (
        <div className="fixed z-[999] w-screen px-5 lg:px-12 py-6 flex justify-between bg-gray-300   duration-500 ease-[cubic-bezier(.3,.86,.36,.95)] bg-opacity-0 backdrop-filter backdrop-blur-md" id='navbar' >
            ...
            <div className=" hidden links font-['Neue'] lg:flex gap-10 ">
                {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
                    <AnimatedLink Content={item} key={index} />
                ))}
            </div>
            <div className='text-[#212121] flex justify-center items-center  lg:hidden' >
